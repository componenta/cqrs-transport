...
function jsonSerializerPayload(array $data): string
{
    return json_encode([
...
it('round-trips supported constructor-backed JSON values with the current versioned envelope', function (): void {
    ...
    $reordered = $serializer->deserialize(
        '{"data":{"id":9,"tags":[]},"__componenta_cqrs":1}',
        JsonRoundTripCommand::class,
    );

    expect($decoded)->toBe([
        '__componenta_cqrs' => 1,
        'data' => ['id' => 7, 'tags' => ['one', 'two']],
    ])->and($roundTrip)->toEqual($command)
        ->and($reordered)->toEqual(new JsonRoundTripCommand(9));
});

it('rejects unversioned payloads instead of keeping a legacy wire path', function (): void {
    expect(fn() => (new JsonCommandSerializer())->deserialize(
        '{"id":8,"tags":[]}',
        JsonRoundTripCommand::class,
    ))->toThrow(TransportException::class, 'must use the versioned envelope');
});
...
it('validates decoded fields with strict PHP type semantics before reflection instantiation', function (): void {
    $serializer = new JsonCommandSerializer();
    $command = $serializer->deserialize(jsonSerializerPayload([
        'id' => 12,
        'ratio' => 3,
        'label' => null,
    ]), JsonStrictTypesCommand::class);
    $literal = $serializer->deserialize(jsonSerializerPayload([
        'enabled' => true,
        'disabled' => false,
    ]), JsonLiteralTypeCommand::class);
...
it('accepts null for nullable named scalar and class constructor types', function (): void {
    $command = (new JsonCommandSerializer())->deserialize(jsonSerializerPayload([
        'label' => null,
        'at' => null,
    ]), JsonNullableNamedTypesCommand::class);

    expect($command)->toEqual(new JsonNullableNamedTypesCommand(null, null));
});

it('rejects scalar coercion, invalid union members, and non-nullable null', function (array $data, string $message): void {
    expect(fn() => (new JsonCommandSerializer())->deserialize(
        jsonSerializerPayload($data),
        JsonStrictTypesCommand::class,
...
it('rejects executable capability types before inspecting transported values', function (Closure $operation): void {
    ...
    'callable string' => [
        fn() => (new JsonCommandSerializer())->deserialize(
            jsonSerializerPayload(['callback' => 'system']),
            JsonCallableParameterCommand::class,
        ),
    ],
    'callable array' => [
        fn() => (new JsonCommandSerializer())->deserialize(
            jsonSerializerPayload([
    ...
    'union containing callable' => [
        fn() => (new JsonCommandSerializer())->deserialize(
            jsonSerializerPayload(['callback' => 'safe']),
            JsonCallableUnionCommand::class,
        ),
    ],
    'nullable Closure' => [
        fn() => (new JsonCommandSerializer())->deserialize(
            jsonSerializerPayload(['callback' => null]),
...
it('fails fast for unsupported command shapes and unknown payload fields', function (Closure $operation, string $message): void {
    ...
    [fn() => (new JsonCommandSerializer())->deserialize(
        jsonSerializerPayload(['id' => 1, 'tags' => [], 'static' => 'leak']),
        JsonRoundTripCommand::class,
    ), 'unknown field'],
]);
