...
it('round-trips supported constructor-backed JSON values with a versioned envelope', function (): void {
    ...
    $reorderedEnvelope = $serializer->deserialize(
    ...
    expect($decoded)->toBe([
        ...
        ->and($reorderedEnvelope)->toEqual(new JsonRoundTripCommand(9))
...
it('ignores public static properties instead of leaking them into payloads', function (): void {
    $serializer = new JsonCommandSerializer();
    $payload = $serializer->serialize(new JsonStaticPropertyCommand(9));

    expect(json_decode($payload, true, flags: JSON_THROW_ON_ERROR)['data'])
        ->toBe(['id' => 9]);
});
...
it('rejects hooked properties without invoking their getters', function (): void {
    ...
    expect(fn() => $serializer->serialize(new JsonHookedPropertyCommand('input')))
...
it('validates decoded fields with strict PHP type semantics before reflection instantiation', function (): void {
    ...
    $command = $serializer->deserialize(
        '{"id":12,"ratio":3,"label":null}',
        JsonStrictTypesCommand::class,
    );
    $literal = $serializer->deserialize(
...
it('rejects scalar coercion and invalid union members from transported JSON', function (
    string $payload,
    string $message,
): void {
    ...
    'integer is not a string or boolean' => [
        '{"id":12,"ratio":3,"label":1}',
        'must match string|bool|null; int given',
    ],
    'boolean is not a float' => [
        '{"id":12,"ratio":true,"label":null}',
...
it('fails fast for object state, private state, variadics, and unknown payload fields', function (
    Closure $operation,
    string $message,
): void {
    ...
    'object value' => [
        fn() => (new JsonCommandSerializer())->serialize(
            new JsonObjectPropertyCommand(new DateTimeImmutable()),
        ),
        'configure a custom serializer',
    ],
    'private constructor state' => [
        fn() => (new JsonCommandSerializer())->serialize(new JsonPrivateStateCommand('secret')),
        'to be public',
    ],
    'variadic constructor' => [
        fn() => (new JsonCommandSerializer())->serialize(new JsonVariadicCommand('a', 'b')),
        'variadic or by-reference',
    ],
    'unknown legacy field' => [
        fn() => (new JsonCommandSerializer())->deserialize(
            '{"id":1,"tags":[],"static":"leak"}',
            JsonRoundTripCommand::class,
        ),
        'unknown field',
    ],
