...
function workerTestKnownCommands(bool $known = true): CommandMetadataProviderInterface
{
    return new class ($known) implements CommandMetadataProviderInterface {
        public function __construct(private readonly bool $known) {}
        public function get(object|string $command, string $attribute): ?object { return null; }
        public function isKnown(object|string $command): bool { return $this->known; }
    };
}
...
it('re-dispatches transported commands with worker attributes', function (): void {
    ...
    $bus = new class implements CommandBusInterface {
        ...
        /** @var array<string, mixed> */ public array $attributes = [];
        public function dispatch(object $command, array $attributes = []): OperationInterface { $this->command = $command; $this->attributes = $attributes; return Operation::create($command, $attributes); }
    };
    ...
    expect($worker->processOne())->toBeTrue()
        ...
        ->and($transport->acks)->toBe(1)->and($transport->rejects)->toBe(0);
});

it('keeps sync execution mode authoritative over custom dispatch attributes', function (): void {
    $envelope = new Envelope('operation-id', stdClass::class, '{}', 'receipt-id');
    $bus = new class implements CommandBusInterface {
        /** @var array<string, mixed> */ public array $attributes = [];
        public function dispatch(object $command, array $attributes = []): OperationInterface { $this->attributes = $attributes; return Operation::create($command, $attributes); }
    };
    $serializer = new class implements CommandSerializerInterface {
        public function serialize(object $command): string { return '{}'; }
        public function deserialize(string $payload, string $commandClass): object { return new stdClass(); }
