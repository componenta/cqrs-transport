...
function workerDispositionEnvelope(): Envelope
{
    return new Envelope('operation-id', stdClass::class, '{}', 'receipt-id');
}
...
it('preserves processing and reject failures when disposition fails', function (): void {
    $processingFailure = new RuntimeException('handler failed');
    $dispositionFailure = new RuntimeException('reject unavailable');
    $transport = new WorkerDispositionTestTransport(
        workerDispositionEnvelope(),
        rejectFailure: $dispositionFailure,
    );
    $worker = new CommandWorker(
        new WorkerDispositionTestBus($processingFailure),
        new WorkerDispositionTestSerializer(),
        $transport,
    );
    ...
    try {
        ...
        expect($exception->processingFailure)->toBe($processingFailure)
            ->and($exception->dispositionFailure)->toBe($dispositionFailure)
            ->and($exception->getPrevious())->toBe($processingFailure)
...
it('rejects an envelope when a custom deserializer returns the wrong command type', function (): void {
    $transport = new WorkerDispositionTestTransport(
        new Envelope('operation-id', DateTimeImmutable::class, '{}', 'receipt-id'),
    );
    $worker = new CommandWorker(
        new WorkerDispositionTestBus(),
        new WorkerDispositionTestSerializer(),
